...
pub fn walk_handles<P: PhysicalMemoryProvider>(
    ...
    todo!("walk_handles not yet implemented")
}
...
mod tests {
    ...
    fn build_process_with_handles(
        ...
        // ObTypeIndexTable symbol vaddr
        ob_type_index_table_vaddr: u64,
        ...
        ptb = ptb.write_phys(
            handle_table_paddr + 0x3C,
            &next_handle.to_le_bytes(),
        );
        ...
        for &(obj_header_vaddr, _granted_access, type_index) in handles {
            ...
            ptb = ptb
                ...
                .write_phys(
                    obj_header_paddr + OBJ_HEADER_TYPE_INDEX,
                    &[type_index],
                );
        }
        ...
        for &(type_index, type_name, obj_type_vaddr) in type_table {
            let slot_offset = (type_index as u64) * 8;
            ...
            ptb = ptb
                ...
                .write_phys_u64(
                    obj_type_paddr + OBJ_TYPE_NAME + 8,
                    name_buf_vaddr,
                )
    ...
    fn walks_single_process_handles() {
        ...
        let ob_table_vaddr: u64 = 0xFFFF_8000_0040_0000;
    ...
    fn skips_zero_object_pointer_entries() {
        ...
        let ptb = PageTableBuilder::new()
            ...
            .map_4k(obj_header_vaddr, obj_header_paddr, flags::WRITABLE)
            .map_4k(obj_type_vaddr, obj_type_paddr, flags::WRITABLE)
            .map_4k(ob_table_vaddr, ob_table_paddr, flags::WRITABLE)
            // PsActiveProcessHead
            .write_phys_u64(head_paddr, eproc_vaddr + EPROCESS_LINKS)
            .write_phys_u64(head_paddr + 8, eproc_vaddr + EPROCESS_LINKS)
            // _EPROCESS
            ...
            .write_phys_u64(entry_page_paddr + 2 * ENTRY_SIZE + ENTRY_OBJECT_PTR, obj_header_vaddr)
            .write_phys(
            ...
            // ObTypeIndexTable
            .write_phys_u64(ob_table_paddr + (type_index as u64) * 8, obj_type_vaddr)
            // _OBJECT_TYPE.Name
