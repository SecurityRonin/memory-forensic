...
pub fn walk_dns_cache<P: PhysicalMemoryProvider>(
    ...
    let Some(hash_table_addr) = hash_table_addr else {
        // No DNS cache symbol found — not an error, just no data available
        return Ok(Vec::new());
    };

    // Read the hash table bucket count (first field or known constant)
    ...
    // Buckets array starts after the count field (or at a known offset)
...
fn read_wide_string<P: PhysicalMemoryProvider>(
    ...
    // Read a chunk of raw bytes — 2 bytes per code unit
...
fn read_cache_entry<P: PhysicalMemoryProvider>(
    ...
    // Read the domain name — Name field is an LPWSTR (pointer to null-terminated UTF-16LE)
    let name = {
    ...
    // Read data length and data pointer for the resolved value
...
mod tests {
    ...
    fn build_dns_reader() -> ObjectReader<SyntheticPhysMem> {
        ...
        // Hash table page: BucketCount=2, Buckets[0]=ENTRY_VADDR, Buckets[1]=0
        ...
        // Name: "evil.c2.example.com" as null-terminated UTF-16LE
