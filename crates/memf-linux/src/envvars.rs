...
mod tests {
    ...
    fn make_test_reader(
        ...
        let isf = IsfBuilder::new()
            ...
            // mm_struct with env_start/env_end
    ...
    fn walk_single_process_envvars() {
        ...
        data[0x240..0x248].copy_from_slice(&env_vaddr.to_le_bytes()); // env_start
        let env_data = b"HOME=/root\0PATH=/usr/bin:/bin\0SHELL=/bin/bash\0";
        let env_end = env_vaddr + env_data.len() as u64;
        data[0x248..0x250].copy_from_slice(&env_end.to_le_bytes()); // env_end
